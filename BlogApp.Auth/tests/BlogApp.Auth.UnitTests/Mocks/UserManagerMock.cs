...
namespace BlogApp.Auth.UnitTests.Mocks;
    ...
    public static Mock<UserManager<UserEntity>> Create(BlogAuthDbContext context)
    {
        ...
        userManager.Setup(x => x.GetRolesAsync(It.IsAny<UserEntity>()))
            .ReturnsAsync((UserEntity user) =>
                context.Roles
                    .Where(c => context.UserRoles.Where(c => c.UserId == user.Id).Select(c => c.RoleId).Contains(c.Id))
                    .Select(c => c.Name!).ToList());
        userManager.Setup(x => x.RemoveFromRolesAsync(It.IsAny<UserEntity>(), It.IsAny<IEnumerable<string>>()))
            .ReturnsAsync(IdentityResult.Success)
            .Callback<UserEntity, IEnumerable<string>>((x, y) =>
                context.UserRoles.Remove(context.UserRoles.Where(a =>
                        a.UserId == x.Id && a.RoleId.Equals(context.Roles.Where(c => c.Name == y.First()).First().Id))
                    .First()).Context.SaveChanges());
        userManager.Setup(x => x.AddToRoleAsync(It.IsAny<UserEntity>(), It.IsAny<string>()))
            .ReturnsAsync(IdentityResult.Success)
            .Callback<UserEntity, string>((x, y) => context.UserRoles.Add(new IdentityUserRole<Guid>
                { RoleId = context.Roles.Where(c => c.Name == y).First().Id, UserId = x.Id }).Context.SaveChanges());
        ...
        userManager.Setup(x => x.GetUsersInRoleAsync(It.IsAny<string>()))
            .ReturnsAsync((string roleName) => context.Users.Where(c =>
                context.UserRoles.Where(c => c.RoleId == context.Roles.First(c => c.Name == roleName).Id)
                    .Select(c => c.UserId).ToList().Contains(c.Id)).ToList());
